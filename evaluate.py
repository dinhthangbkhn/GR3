...
def evaluate_model(model, train_data, test_data, cut_off=20, batch_size=100):
    model.predict = False

    test_data.sort_values(['SessionId', 'Time'], ascending=True)
    offset_sessions = np.zeros(test_data['SessionId'].nunique() + 1, dtype=np.int32)
    offset_sessions[1:] = test_data.groupby('SessionId').size().cumsum()
